...
public class GraphUIManager : MonoBehaviour
{
    ...
    public TMP_Dropdown robotProgramDropdown;
    ...
    void Start()
    {
        ...
        foreach (var item in toolBoxItems)
        {
            ...
            currentsprite.GetComponent<toolBoxItem>().itemImg = item.toolSignifier;
    ...
    private void Update()
    {
        ...
        if (toolIsSelected == true && graphManager.stateMachine.InteractionMode.tool == selectedTool.none)
        {
            ...
            mouseItem.GetComponent<Image>().enabled = false;
            graphManager.audioManager.Play("UnSelectTool");
            
        }else if (toolIsSelected)
        {
            mouseItem.GetComponent<Image>().sprite = selectedToolOption.GetComponent<toolBoxItem>().itemImg;
        }
    ...
    public void changeRobotProgram()
    {
        switch (robotProgramDropdown.options[robotProgramDropdown.value].text)
        {
            case "Prims Algorithm":
                graphManager.stateMachine.InteractionMode.robot.programType = ProgramType.prims;
                break;
            case "Kruskal's algorithm":
                graphManager.stateMachine.InteractionMode.robot.programType = ProgramType.kruskals;
                break;
        }
    }
